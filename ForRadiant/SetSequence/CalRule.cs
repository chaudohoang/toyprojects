...
namespace SetSequence
{
    ...
    public partial class CalRule : Form
    {
        ...
        public List<string> sequenceAnaList = new List<string>();
        public XmlNode node3;
        public XmlNodeList nodes3;
        public XmlDocument xmlDoc3 = new XmlDocument();
        ...
        public object GetConnect(string CameraSN)
        {
            string calFile1 = Path.Combine(@"C:\Radiant Vision Systems Data\Camera Data\Calibration Files", CameraSN + "_CalibrationDB.calx");
            string calFile2 = Path.Combine(@"C:\Radiant Vision Systems Data\Camera Data\Calibration Files", "0" +CameraSN + "_CalibrationDB.calx");
        ...
        {
            ReloadColorCalRule2();
            ReloadFlatFieldCalRule2();
            ReloadImgScaleCalRule2();
        }
        ...
        private void ReloadColorCalRule2()
        {
            string Sequence = txtFile3.Text;
            XmlNode node;
            XmlNodeList nodes;
            var ColorCalSetting = new List<string>();
            var ImgScaleSetting = new List<string>();
            var FFCSetting = new List<string>();

            var xmlDoc = new XmlDocument();

            xmlDoc.Load(Sequence);
            ...
            for (int i = 0, loopTo = nodes.Count - 1; i <= loopTo; i++)
            {
                ...
                {
                    sequence1AnaList.Add(nodes[i].SelectSingleNode("PatternSetupName").InnerText);
                }
            ...
            for (int index = 0, loopTo1 = nodes.Count - 1; index <= loopTo1; index++)
            {
                ...
                if (sequence1AnaList.Contains(nodes[index].SelectSingleNode("Name").InnerText))
                {
                    ...
                    string CCID = node.SelectSingleNode("CameraSettings/ColorCalID").InnerText;

                    ColorCalSetting.Add(nodes[index].SelectSingleNode("Name").InnerText + "," + CCID);
            ...
            for (int index = 0, loopTo = Cols - 1; index <= loopTo; index++)
                dt.Columns.Add(Fields[index]);
                DataRow dr;
            for (int i = 0; i < ColorCalSetting.Count -1; i++)
            {
                Fields = ColorCalSetting[i].Split(',');
                dr = dt.NewRow();
                for (int f = 0, loopTo2 = Cols - 1; f <= loopTo2; f++)
        ...
        private void ReloadFlatFieldCalRule2()
        {
            string Sequence = txtFile3.Text;
            XmlNode node;
            XmlNodeList nodes;
            ...
            for (int index = 0, loopTo1 = nodes.Count - 1; index <= loopTo1; index++)
            {
                ...
                if (sequence1AnaList.Contains(nodes[index].SelectSingleNode("Name").InnerText))
                {
                    ...
                    string FFID = node.SelectSingleNode("CameraSettings/FlatFieldID").InnerText;

                    FFCSetting.Add(nodes[index].SelectSingleNode("Name").InnerText + "," + FFID);
            ...
            for (int i = 0; i < FFCSetting.Count - 1; i++)
            {
                Fields = FFCSetting[i].Split(',');
                dr = dt.NewRow();
                for (int f = 0, loopTo2 = Cols - 1; f <= loopTo2; f++)
        ...
        private void ReloadImgScaleCalRule2()
        {
            string Sequence = txtFile3.Text;
            XmlNode node;
            XmlNodeList nodes;
            ...
            for (int index = 0, loopTo1 = nodes.Count - 1; index <= loopTo1; index++)
            {
                ...
                if (sequence1AnaList.Contains(nodes[index].SelectSingleNode("Name").InnerText))
                {
                    ...
                    string CCID = node.SelectSingleNode("CameraSettings/ColorCalID").InnerText;
                    string IMCID = node.SelectSingleNode("CameraSettings/ImageScalingCalibrationID").InnerText;
                    string FFID = node.SelectSingleNode("CameraSettings/FlatFieldID").InnerText;

                    ColorCalSetting.Add(nodes[index].SelectSingleNode("Name").InnerText + "," + CCID);
                    ImgScaleSetting.Add(nodes[index].SelectSingleNode("Name").InnerText + "," + IMCID);
                    FFCSetting.Add(nodes[index].SelectSingleNode("Name").InnerText + "," + FFID);
            ...
            for (int index = 0, loopTo = Cols - 1; index <= loopTo; index++)
                ...
                DataRow dr;
            for (int i = 0; i < ImgScaleSetting.Count - 1; i++)
            {
                Fields = ImgScaleSetting[i].Split(',');
                dr = dt.NewRow();
                for (int f = 0, loopTo2 = Cols - 1; f <= loopTo2; f++)
        ...
        private void btnColorCalSaveCalRules_Click(object sender, EventArgs e)
        {
            ...
            string ColorCalRuleFilaName = Path.GetFileNameWithoutExtension(txtFile3.Text) + "_colorcal.txt";
            string ColorCalRuleFilePath = Path.Combine(@"C:\Radiant Vision Systems Data\TrueTest\Sequence\Calibration Rules", ColorCalRuleFilaName);
            var rows = from DataGridViewRow row in ColorCalDataGridView2.Rows.Cast<DataGridViewRow>()
                       where !row.IsNewRow
            ...
            if (rowsData.Count > 0)
            {
                saveContent = string.Join(System.Environment.NewLine, rowsData);
            }
