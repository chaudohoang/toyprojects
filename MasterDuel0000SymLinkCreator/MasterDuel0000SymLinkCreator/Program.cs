...
namespace MasterDuel0000SymLinkCreator
{
    class SymlinkCreator
    {
        static void Main()
        {
            ...
            int count = 0;
            Regex hexPattern = new Regex("^[a-fA-F0-9]{8}$");

            foreach (var dir in directories)
            {
                string folderName = Path.GetFileName(dir);

                // Check if folder name is exactly 8 hex characters
                if (hexPattern.IsMatch(folderName))
                {
                    ...
                    if (Directory.Exists(symlinkPath) || File.Exists(symlinkPath))
                    {
                        ...
                        else
                        {
                            ...
                            Directory.CreateSymbolicLink(symlinkPath, masterFolderFullPath);
